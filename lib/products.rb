...
class ProductsByUpc < GraphQL::Dataloader::Source
  def fetch(upcs)
    puts "ProductsByUpc.fetch #{upcs}"
    upcs.map do |upc|
...
class Product < BaseObject
  ...
  def self.resolve_reference(reference, context)
    puts "Product.resolve_reference reference: #{reference}"
    context.dataloader.with(ProductsByUpc).load(reference[:id])
  end
end
